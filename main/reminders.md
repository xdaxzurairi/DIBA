...
- **Scheduled health-check task tak boleh jalan (2026-09-09):** Scheduled routine "health check across connected monitoring services" tiada apa nak check — takde connector monitoring/incident (Sentry/Datadog/PagerDuty/UptimeRobot/StatusPage) setup langsung. Vercel & Supabase connected kat org level tapi `enabledInChat: false` untuk session ni, so deployment/db status pun tak boleh query. GitHub Actions repo DIBA sendiri sihat (3 runs, last Aug 19, no active workflow relevant to "incidents"). Kalau nak scheduled health check betul-betul jalan: (1) enable Vercel + Supabase MCP untuk session automation, atau (2) connect proper monitoring/status tool. Rujuk balik skop scheduled task ni dengan Abam.
