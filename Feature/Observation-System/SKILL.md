...
Skill ni **milik DIBA**. Di mana Claude Code ada tool native, guna ia sebagai **enjin**, lepas tu balut dengan rawatan DIBA:

| Tier | Enjin native | Rawatan DIBA di atasnya |
|---|---|---|
| **Refine** (kod diubah) | `/code-review` (bug) + `/simplify` (quality) | Verdict rojak lead-with-finding · log finding non-obvious ke `main/decisions.md` · chain `save-diary` |
| **Audit** (sistem penuh) | `/code-review` skop luas + `repo-pack`/`project-map` | Sintesis DIBA: risk table, owner, next step — bukan dump mentah |
| **Survey / Investigate** | (DIBA sendiri — grep, read, trace) | — |

Kalau tool native tak wujud pada mesin tu, fallback ke protokol manual di bawah. DIBA kekal pintu depan; native tool cuma detail implementasi.
